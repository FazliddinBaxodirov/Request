...
.then(data => {
    data.forEach(emt => {
        ...
        elUserItem.innerHTML = `
            ...
            <button onclick="findPost(${emt.id})" class="p-[5px] bg-violet-600 rounded-md w-full mt-2 text-white">Show posts</button>
        `
...
function findPost(id){
    ...
    .then(data => {
        ...
        data.forEach(item => {
            ...
            elPostItem.className = "w-[400px] bg-white rounded-md p-5"
            elPostItem.innerHTML = `
                ...
                <button onclick="findComment(${item.id})" class="p-[5px] bg-violet-600 rounded-md w-full mt-2 text-white">Show comments</button>
            `
    ...
    .then(data => {
        ...
        data.forEach(item => {
            ...
            elCommentItem.innerHTML = `
                <p class="text-violet-400">PostID: <strong class="text-violet-600">${item.postId}</strong></p>
                <p class="mt-[5px] text-violet-400">Id: <strong class="text-violet-600">${item.id}</strong></p>
                <p class="mt-[5px] text-violet-400">Name: <strong class="text-violet-600">${item.name}</strong></p>
                <p class="mt-[5px] text-violet-400">Email: <strong class="text-violet-600">${item.email}</strong></p>
                <p class="mt-[5px] text-violet-400">Body: <strong class="text-violet-600">${item.body}</strong></p>
            `
